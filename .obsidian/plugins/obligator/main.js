  ...
  async onload() {
    ...
    const run_obligator = async () => {
      ...
      if (this.settings.terminal === "") {
        new import_obsidian4.Notice("You must specify the terminal heading in the settings.");
        return;
      }
      ...
      if (last_note) {
        ...
        if (last_note_terminal_index === -1) {
          new import_obsidian4.Notice(`${last_note.basename} does not contain the
      ...
      if (this.settings.archive && last_note) {
        ...
        try {
          ...
          new import_obsidian4.Notice(`A file called ${archived_note_path} already exists, archival skipped.`);
        }
      }
    };
...
var ObligatorSettingTab = class extends import_obsidian4.PluginSettingTab {
  ...
  async getHeadings(file_path) {
    ...
    const headings = Array.from(content.matchAll(HEADING_REGEX_GLOBAL)).reduce((accumulator, [heading], index) => {
  ...
  async display() {
    ...
    containerEl.empty();
    new import_obsidian4.Setting(containerEl).setName("New file location").setDesc("New daily notes will be placed here.").addText((text) => {
    ...
    new import_obsidian4.Setting(containerEl).setName("Terminal Heading").setDesc(`The heading from the template which terminates what Obligator will copy. Everything between the start of the note and this heading in your daily note will be copied over.`).addDropdown(
    ...
    new import_obsidian4.Setting(containerEl).setName("Archive old notes").setDesc(`Enabling this will move the previous to-do note into the
